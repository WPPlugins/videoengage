	...
	*/
	...
	$('.play-button-pulse').each(function() {
		if ( $( this ).attr( 'data-source' ) == 'vimeo' ) // check if video is youtube or self-hosted
		{
			...
			var vimeoAutoplay = $( this ).attr( 'data-autoplay' );
			...
			*/
			...
			vimeoAutoplayArray[ counter ] = vimeoAutoplay;
    ...
    var player = [];
      ...
      postIdArray.forEach(function( wert, index, referenz ) {
		...
		var iframe = $( '#vimeo-iframe-' + wert );
		player[index] = new Vimeo.Player(iframe);
			...
			//var player[index] = new Vimeo.Player('yt-iframe-player-' + wert, options );
			player[index] = new Vimeo.Player('vimeo-video-player-' + wert, options);
			*/
	...
	autoplay(); // look for autoplay videos
	  ...
	  // This function is called by initialize()
		function updateTimerDisplay() {
			postIdArray.forEach(function( wert, index, referenz ) {
					...
					player[index].getCurrentTime().then(function(seconds) {
						currentTime = formatTime(seconds);
					}).catch(function(error) {
						// an error occurred
					});
					
					player[index].getDuration().then(function(seconds) {
						videoDuration = formatTime(seconds);
					}).catch(function(error) {
						// an error occurred
					});
					...
					var diff = videoDuration + durationBuffer;
					endImage = $( "#play-button-pulse-" + wert ).attr( "data-background-end-image" );
					//console.log( currentTime + ' ' + diff + ' ' + $( "#play-button-pulse-" + wert ).attr('data-background-end-image') );
					if ( currentTime >= diff && currentTime != 0 && endImage != '' ) {
						// thumbnail name
						var thumbnailName = '.yt-thumbnail-' + wert + $( "#play-button-pulse-" + wert ).attr( 'data-imname' );
						// hide yt player
						$( '#vimeo-iframe-' + wert ).css( 'display', 'none' );
					...
					if ( showCtaArray[ index ] == 'true' && ctaDoneArray[ index ] == false  && ctaTimeArray[ index ] != currentTime && currentTime != 0 ) // check if cta is on and was not shown
					{
						...
						{
							if ( ctaStopArray[index] == 'true' ) {
								player[index].pause(); // pause player
...
// end youtube api
		...
		function autoplay() {
			...
			vimeoAutoplayArray.forEach(function( wert, index, referenz ) {
				...
				if ( wert == 'true' )
				{
					...
					// show video
					$( '#vimeo-iframe-' + postId ).css( 'display', 'block' );
					$( 'i#play-button-' + postId ).hide();
					$( '#play-button-pulse-' + postId ).hide();
					
					// if no stop window & mobile device then display window to control play & stop on mobile devices
					var mobileDevice = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
					if ( $( '#yt-nostop-window-' + postId ).attr( 'data-stop' ) == 'true' )
					{
						if ( mobileDevice ) {
							// play Video
							//player[ PlayerNr ].play();
							$( '#vimeo-container-' + postId ).css( 'display', 'block' );
		...
		function CloseButtonDisplay( index, wert, duration, contentNr ) {
			$( 'i#vimeo-close-' + wert ).show(); // show close button by default
			$( 'i#vimeo-close-' + contentNr + '-' + wert ).show(); // show close button by default
			if ( closeButtonCtaArray[index] == 'true' )
			{
				if ( duration != 0) // cta will disappear automatically
				{
					$( 'i#vimeo-close-' + wert ).hide(); // hide close button
					$( 'i#vimeo-close-' + contentNr + '-' + wert ).hide(); // hide close button of sites or articles					
				}
			}
		}
		...
		function autoDelayFunction( wert, index, duration, number ) {
			...
			if ( eval( "ctaAudio" + number + "Array[index]" ) !=""  && vimeoAutoplayArray[index] != 'true' )
			{
				...
				player[index].pause(); // pause video
				ctaTimeArray[index] = currentTime;
			}
			if ( duration != 0 )
			{
	...
	$( '.play-button-pulse' ).on('click', function() {
		if ( $( this ).attr( 'data-source' ) == 'vimeo' ) // check if video is youtube or self-hosted
		{
			...
			var vimeoAutoplay = '?rel=0&amp;autoplay=1';
			
			if ( $( '.cta-display' ).attr( 'data-autoplay' ) == 'true' )
				vimeoAutoplay = '?rel=0&amp;autoplay=1';
	...
	/* end of responsive settings */
		...
		$( '.vimeo-container' ).on('click', function() {
			...
			postIdArray.forEach(function( wert, index, referenz ) {
				...
				if ( wert == postId )
				{
					...
					player[index].getPaused().then(function(paused) {
						if (paused)
							player[index].play();
						else
							player[index].pause();
